...
namespace rigid2d
{
    ...
    struct Vector2D
    {
        ...
        double y;
        Vector2D();
        explicit Vector2D(double a, double b);
    ...
    struct Twist2D
    {
        ...
        double Vy = 0.0;
        double Wz = 0.0;
        Twist2D & operator+=(const Twist2D & t);
        Twist2D operator+(const Twist2D & t2);
        Twist2D & operator-=(const Twist2D & t);
        Twist2D operator-(const Twist2D & t2);
        Twist2D operator*(double a);
        Twist2D operator/(double a);
    };

    /// \brief output a 2 dimensional twist as [Wz Vx Vy]
    ...
    public:
        ...
        // \compute the displacement of the transform matrix
        // \return the x, y between origins and the rotation angle
    ...
    Transform2D integrateTwist(const Twist2D & t);
}

...
static bool next_cmd_flag=1;
static int frequency=100;
static rigid2d::Twist2D expected_pose,pose_error,ac_pose_error,d_pose_error,past_pose_error;
static rigid2d::waypoints traj;
static std::vector<rigid2d::Twist2D> expPoseSerial;
...
void timerCallback(const ros::TimerEvent& event)
{
  ...
  if (flag_way==1)
  {
    ...
    {
      /// \brief use pid controller
      expected_pose=traj.getpose();
      expPoseSerial.erase(expPoseSerial.begin());
      expPoseSerial.push_back(expected_pose);

      double x,y,theta;
      theta=actual_pose.Wz;
      x=expPoseSerial[0].Vx-actual_pose.Vx;
      y=expPoseSerial[0].Vy-actual_pose.Vy;
      rigid2d::Vector2D diffb;
      rigid2d::Twist2D Vb;
      ...
      /// \brief just use actual_pose and forward control
      // Vb=traj.nextWaypointfromActualPose(actual_pose);
      // Vb.Vx*=frequency;
      // Vb.Wz*=frequency;
  ...
  // ROS_INFO("actual pose x %f y %f wz %f",actual_pose.Vx,actual_pose.Vy,actual_pose.Wz);
  // ROS_INFO("cmd Vx %f Wz %f",cmd.linear.x,cmd.angular.z);
}
...
int main(int argc, char **argv)
{
  ...
  traj.setParam(way_p,max_Vx, max_Wz,1.0/frequency);
  ...
  rigid2d::Twist2D temppose;
  temppose.Vx=way_p[0].x;
  temppose.Vy=way_p[0].y;
  temppose.Wz=0;
  for (int i=0;i<3;i++)
  {
    expPoseSerial.push_back(temppose);
  }
  ...
  /// \brief create a timer to pub cmd.
  ros::Timer timer = n.createTimer(ros::Duration(1.0/frequency), timerCallback);
  ros::spin();

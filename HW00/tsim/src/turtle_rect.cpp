...
turtlesim::TeleportAbsolute lb_cor;
int stage_count=2;
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  if (thetapose<0) thetapose=2*PI+thetapose;
  if (thetanow<0) thetanow=2*PI+thetanow;
  theta_error0 = thetapose-thetanow;
  theta_error = abs(theta_error0);
  /**absolute angle_error should belong to (0,PI).*/
  if (theta_error>PI) theta_error=2*PI-theta_error;
  ...
  else
  {
    ...
    cmd_v.linear.x=0;
    if (theta_error0>=0&&theta_error0<PI)
    {
      cmd_v.angular.z=rot_vel;
    }
    else if (theta_error0>=PI)
    {
      cmd_v.angular.z=-rot_vel;
    }
    else if (theta_error0<=0&&theta_error0>-PI)
    {
      cmd_v.angular.z=-rot_vel;
    }
    else if (theta_error0<=-PI)
